...
コンテキスト（Knowlege,Playbooks）
1. プロジェクトコンテキスト
	- プロジェクト構造、技術スタック、アーキテクチャ、ルールなど
2. 業務コンテキスト
	- ドメイン、制約など
...
#### 前提
エンジニアリング知識があるメンバーが使用する


### 修正ファイル数（1〜3）

| 用途                          | コンテキスト | 最低限のコンテキスト               | プロンプト |
| --------------------------- | ------ | ------------------------ | ----- |
| UI（Template, Sscript）       | △      | コーディングルール                |       |
| UI（Stype Block）             | ◎      | 視覚的なゴール<br>（Figma MCPなど） |       |
| UI（Component Test）          | ○      | テストルール                   |       |
| API（Root Handler）           | △      | コーディングルール                |       |
| API（Controller, Repository） | ◎      | プロジェクト構成、技術スタック、ルール      |       |
| ==API（Test）==               | -      | -                        |       |
| DB（スキーマ）                    |        |                          |       |
| DB（SQL）                     |        |                          |       |
| FR                          | ◎      | 業務コンテキスト、手順書             |       |
...
| 用途                          | コンテキスト | 最低限のコンテキスト               |
| --------------------------- | ------ | ------------------------ |
| UI（Template, Sscript）       | ○      | アーキテクチャ、コーディングルール        |
| UI（Stype Block）             | ◎      | 視覚的なゴール<br>（Figma MCPなど） |
| API（Root Handler）           | △      | コーディングルール                |
| API（Controller, Repository） | ◎      | プロジェクト構成、技術スタック、ルール      |
| DB（スキーマ）                    |        |                          |
...
| 用途  | コンテキスト | 最低限のコンテキスト                  | 備考            |
| --- | ------ | --------------------------- | ------------- |
| ALL | ◎      | プロジェクト構造、技術スタック、アーキテクチャ、ルール | AIモデルの性能に依存する |



![](Pasted%20image%2020250811222652.png)