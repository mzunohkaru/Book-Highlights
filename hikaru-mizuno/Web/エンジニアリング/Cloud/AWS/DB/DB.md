...
- 接続リークの発生メカニズム
```
1. Lambda関数実行 → DB接続作成（例：connection = mysql.connect()）
2. 関数処理完了 → Lambdaは「完了」を返すが、コンテナは生存
3. DB接続オブジェクトはグローバル変数として残存
4. 次回実行時 → 新しい接続を作成（古い接続は放置）
5. 繰り返しにより、未使用の接続が蓄積
```

**RDS Proxyによる解決**
- 接続の抽象化 RDS Proxyは、Lambda関数とRDSインスタンス間にプロキシレイヤーを挿入します。Lambda関数は実際にはRDS Proxyに接続し、Proxyが背後でRDSとの物理接続を管理します。
- 自動接続管理 Lambda関数が接続を「クローズ」した場合、RDS Proxyはその接続をプールに戻します。関数が接続をクローズし忘れても、Proxyは一定時間後（デフォルト1800秒）に自動的に接続をクリーンアップします。


**マルチプレキシング** 複数のLambda関数からの接続要求を、少数の物理的なRDS接続で処理できます。例えば、100個のLambda関数が同時実行されても、RDSへの物理接続は10-20程度に抑制されます。
